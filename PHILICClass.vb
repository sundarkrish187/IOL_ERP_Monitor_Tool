...
Public Class PHILICClass

    Dim con As New SqlConnection
    Dim conPHILIC, conPHILIC_ERP, conStrPHOBIC_ERP As New SqlConnection
    Public selectedDate As DateTime
    Public Total As Integer

    Public Sub PHILICClass()
        ...
        conStrPHOBIC_ERP.ConnectionString = ConfigurationSettings.AppSettings("conStrPHOBIC_ERP").ToString()
    End Sub
    ...
    Public Function GetNameList(ByVal byname As String) As List(Of String)
        ...
        strsql = "SELECT  Distinct " + byname + " FROM dbo.View_PHILIC_Monitor where boxtime BETWEEN '" + selectedDate.ToString("yyyy-MM-dd") + " 06:00:00.000' AND '" + selectedDate.ToString("yyyy-MM-dd") + " 22:00:00.000'  "
        Dim cmd As New SqlCommand(strsql, conPHILIC)
    ...
    Public Function GetSummaryHourlyTable(ByVal lst As List(Of String), ByVal byname As String) As DataTable
        ...
        For i = 0 To lst.Count - 1
            'Dim ds As New DataSet
            'strsql = "select * from View_PHILIC_Monitor_Hourly where " + byname + " = '" & lst(i) & "' and created_date BETWEEN '" + selectedDate.ToString("yyyy-MM-dd") + "' AND '" + selectedDate.ToString("yyyy-MM-dd") + "' order by cast (hour as int)  "
            ...
            'PHILIC DB
            Dim dsPhilic As New DataSet
            strsql = "select * from View_PHILIC_Monitor_Hourly where " + byname + " = '" & lst(i) & "' and BoxTime BETWEEN '" + selectedDate.ToString("yyyy-MM-dd") + "' AND '" + selectedDate.ToString("yyyy-MM-dd") + "' order by cast (hour as int)  "
            cmd = New SqlCommand(strsql, conPHILIC)
            Dim ad1 = New SqlDataAdapter(cmd)
            ad1.Fill(dsPhilic)

            For k = 0 To dsPhilic.Tables(0).Rows.Count - 1
                If Not IsDBNull(row("" & dsPhilic.Tables(0).Rows(k)("Hour") & "")) Then
                    row("" & dsPhilic.Tables(0).Rows(k)("Hour") & "") = Convert.ToInt32(row("" & dsPhilic.Tables(0).Rows(k)("Hour") & "")) + Convert.ToInt32(dsPhilic.Tables(0).Rows(k)("Qty"))
                Else
                    row("" & dsPhilic.Tables(0).Rows(k)("Hour") & "") = dsPhilic.Tables(0).Rows(k)("Qty")
                End If
                total = total + Convert.ToInt32(dsPhilic.Tables(0).Rows(k)("Qty"))
                row("Total") = total
            Next k
    ...
    Public Function GetSummaryHourlyTableShift1(ByVal lst As List(Of String), ByVal byname As String) As DataTable
        ...
        For i = 0 To lst.Count - 1
            ...
            For k = 0 To dsPhilic.Tables(0).Rows.Count - 1
                If Convert.ToInt32(dsPhilic.Tables(0).Rows(k)("Hour")) > 13 Then Exit For
                If Not IsDBNull(row("" & dsPhilic.Tables(0).Rows(k)("Hour") & "")) Then
                    row("" & dsPhilic.Tables(0).Rows(k)("Hour") & "") = Convert.ToInt32(row("" & dsPhilic.Tables(0).Rows(k)("Hour") & "")) + Convert.ToInt32(dsPhilic.Tables(0).Rows(k)("Qty"))
                Else
                    row("" & dsPhilic.Tables(0).Rows(k)("Hour") & "") = dsPhilic.Tables(0).Rows(k)("Qty")
    ...
    Public Function GetSummaryHourlyTableShift2(ByVal lst As List(Of String), ByVal byname As String) As DataTable
        ...
        For i = 0 To lst.Count - 1
            ...
            For k = 0 To dsPhilic.Tables(0).Rows.Count - 1
                If Convert.ToInt32(dsPhilic.Tables(0).Rows(k)("Hour")) <= 13 Then Continue For
                If Not IsDBNull(row("" & dsPhilic.Tables(0).Rows(k)("Hour") & "")) Then
                    row("" & dsPhilic.Tables(0).Rows(k)("Hour") & "") = Convert.ToInt32(row("" & dsPhilic.Tables(0).Rows(k)("Hour") & "")) + Convert.ToInt32(dsPhilic.Tables(0).Rows(k)("Qty"))
                Else
                    row("" & dsPhilic.Tables(0).Rows(k)("Hour") & "") = dsPhilic.Tables(0).Rows(k)("Qty")
    ...
    Public Function GetSummaryTable(ByVal lst As List(Of String), ByVal byname As String) As DataTable
        ...
        For i = 0 To lst.Count - 1
            ...
            'strsql = "select isnull(sum(qty),0) from View_PHILIC_Monitor where " + byname + " = '" & lst(i) & "' and shift = 'SHIFT-I' and created_date BETWEEN '" + selectedDate.ToString("yyyy-MM-dd") + " 06:00:00.000' AND '" + selectedDate.ToString("yyyy-MM-dd") + " 22:00:00.000'   "
            'cmd = New SqlCommand(strsql, con)
            'Dim ad1 = New SqlDataAdapter(cmd)
            'ad1.Fill(ds_shift1)
            ...
            'strsql = "select isnull(sum(qty),0) from View_PHILIC_Monitor where " + byname + " = '" & lst(i) & "' and shift = 'SHIFT-II' and created_date BETWEEN '" + selectedDate.ToString("yyyy-MM-dd") + " 06:00:00.000' AND '" + selectedDate.ToString("yyyy-MM-dd") + " 22:00:00.000'   "
            'cmd = New SqlCommand(strsql, con)
            ...
            strsql = "select isnull(sum(qty_1),0) from View_PHILIC_Monitor where " + byname + " = '" & lst(i) & "' and shift = 'SHIFT-I' and boxtime BETWEEN '" + selectedDate.ToString("yyyy-MM-dd") + " 06:00:00.000' AND '" + selectedDate.ToString("yyyy-MM-dd") + " 22:00:00.000'   "
            cmd = New SqlCommand(strsql, conPHILIC)
            ...
            strsql = "select isnull(sum(qty_1),0) from View_PHILIC_Monitor where " + byname + " = '" & lst(i) & "' and shift = 'SHIFT-II' and boxtime BETWEEN '" + selectedDate.ToString("yyyy-MM-dd") + " 06:00:00.000' AND '" + selectedDate.ToString("yyyy-MM-dd") + " 22:00:00.000'   "
            cmd = New SqlCommand(strsql, conPHILIC)
...
"FROM  (SELECT Model_name, Power, BlanksType, SUM(shiftI) AS shift1_qty, SUM(shiftII) AS shift2_Qty, SUM(Accepted_Qty) AS Acc_Qty, SUM(Rejected_Qty) AS Rej_Qty FROM dbo.CuttingDetails_FirstCut  " & _
        ...
        Dim cmd As New SqlCommand(strsql, conPHILIC_ERP)
        Dim ad As New SqlDataAdapter(cmd)
        ad.Fill(ds)
        lst.Clear()
        For i = 0 To ds.Tables(0).Rows.Count - 1
...
"FROM  (SELECT Model_name, Power, BlanksType, SUM(shiftI) AS shift1_qty, SUM(shiftII) AS shift2_Qty, SUM(Accepted_Qty) AS Acc_Qty, SUM(Rejected_Qty) AS Rej_Qty FROM dbo.CuttingDetails_SecondCut  " & _
    ...
    Public Function GetFirstCutReport(ByVal byModel As String, ByVal byBlanks As String) As DataTable
        ...
        cmd = New SqlCommand(strsql, conPHILIC_ERP)
        Dim ad As New SqlDataAdapter(cmd)
        ad.Fill(ds)
        dt = ds.Tables(0)
